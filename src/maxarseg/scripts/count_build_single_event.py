...
def main():
    ...
    for mos_name in mos_names: #only mos in lbl
        ...
        for tile_name in tif_names:
            current_tif += 1
            print(f'{ev_name}/pre/{mos_name}/{tile_name}, tif:({current_tif}/{len(tif_names)}), mos:({current_mos}/{len(mos_names)})', flush=True)
    ...
    np.save(f'{ev_name}_high_res_entropies.npy', high_res_entropies_np)
    print(f'Saved {ev_name}_high_res_entropies.npy', flush=True)
    res_df = pd.DataFrame(cols)
    res_df.to_csv(f'{ev_name}_lbl_stats.csv', index = True)
    print(f'Saved {ev_name}_lbl_stats.csv', flush=True)
