...
#used when you want to segment rnd samples from your tile
...
def evit_get_batch_box_prompt(patch_bbox: BoundingBox, proj_gdf: gpd.GeoDataFrame, dataset_res, ext_mt = 0) -> torch.Tensor:
    """
    Given a bounding box in a proj crs, it returns the boxes in the right coordinates relative to the sampled patch.
    It is necessary that the patch_bbox is in the same crs as the gdf.
    """
    query_patch_poly = samplers_utils.boundingBox_2_Polygon(patch_bbox) #from patch bbox to polygon
    gdf_index = proj_gdf.sindex #get spatial index
    hits = gdf_index.query(query_patch_poly) #query index
